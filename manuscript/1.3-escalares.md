...
En muchos casos, necesitaremos también un comportamiento asociado, como sería el caso de un `OrderStatus` que nos permita saber si un pedido está pendiente, enviado o entregado, para determinar si podemos hacer progresar ese pedido a una nueva fase. En esos casos, lo habitual es consultar de qué estado se trata para decidir qué hacer a continuación. En estos casos, lo que necesitamos es un objeto que encapsule el estado y el comportamiento asociado, lo que se conoce como patrón _State_, del cual hablaremos extensamente en el capítulo correspondiente.

La línea que separa el uso de un tipo Enumerable y un objeto _State_ es bastante difusa, y en muchos casos se solapan. En general, si necesitamos un objeto que represente un estado y tenga comportamiento asociado, lo que necesitamos es un patrón _State_, representando los distintos estados no ya como valores, sino como objetos. Si solo necesitamos representar un valor que puede ser uno de varios posibles, lo que necesitamos es un tipo Enumerable.
...
Los _value objects_ son un tipo de objetos que representan algún concepto importante en el dominio de negocio de la aplicación. Puedes encontrar más detalles y ejemplos en este [artículo de Dani Tomé](https://danitome24.github.io/2018-11-19/usando-value-objects-con-php).
